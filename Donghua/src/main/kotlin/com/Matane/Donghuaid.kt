...
class Donghuaid : Anichin() {
   ...
   override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
    ...
    // ==========================================
    // 🔥 FIX #2: OVERRIDE search — POSTER DARI data-src
    // ==========================================
    ...
    // Prioritas poster: data-src > data-litespeed-src > src (skip data:image/*)
    // ==========================================
    private fun Element.toSearchResultFixed(): SearchResponse? {
        ...
        // Deteksi tipe dari URL atau badge
    ...
    // ==========================================
    // 🔥 FIX #3: OVERRIDE load — POSTER HALAMAN DETAIL + METADATA
    // ==========================================
    override suspend fun load(url: String): LoadResponse {
        ...
        val status = document.selectFirst(".spe:contains(Status), .status")?.text()
            ?.replace("Status:", "")?.trim()
        val typeText = document.selectFirst(".spe:contains(Type), .type")?.text()
            ?.replace("Type:", "")?.trim()
        val duration = document.selectFirst(".spe:contains(Duration), .duration")?.text()
            ?.replace("Duration:", "")?.trim()
        val studio = document.selectFirst(".spe:contains(Studio), .studio")?.text()
            ?.replace("Studio:", "")?.trim()
        val country = document.selectFirst(".spe:contains(Country), .country")?.text()
        ...
        // Genre / Tags
        val tags = document.select(".genres a, .tags a, [class*=genre] a")
            .mapNotNull { it.text().trim().takeIf { t -> t.isNotEmpty() } }
            .ifEmpty {
                document.selectFirst(".spe:contains(Genre)")?.text()
                    ?.replace("Genre:", "")?.trim()
        ...
        if (tvTag == TvType.TvSeries && episodeElements.isNotEmpty()) {
            ...
            return newTvSeriesLoadResponse(title, url, TvType.Anime, episodes.reversed()) {
                ...
                this.status = when {
                    ...
                    status?.contains("Complete", true) == true -> ShowStatus.Completed
                    else -> null
                }
